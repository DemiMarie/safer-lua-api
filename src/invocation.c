...
#endif
#include <stdbool.h>
#define CHECK(x) if (NULL == (x)) { return NULL; } else ((void)0)
...
#endif
/**
 * @brief luaS_call_gate serves as a call gate.  Behavior is
 * undefined unless called by Lua, either as a C function or via
 * lua_pcall/lua_call, etc.  It calls its first upvalue as a
 * @code{lua_CFunction} with the current Lua stack, and inspects the
 * return value:
 *
 ...
 *   Lua error.
 * - If the return value is a negative number @code{x} other than -1,
 *   then @code{(-2) - x} values are popped, and the function yields
 *   this many values to Lua.
 *
 * @param L The @code{lua_State}
 * @return @code{result} if result is non-negative, otherwise never
 * returns.
 */
static int luaS_call_gate(lua_State *L) {
   ...
   for (size_t i = 1; i < func->num_types; ++i) {
      if (LUA_TNONE == func->types[i]) { continue; }
 ...
 * @return 1
 * This function converts the first element of the stack to a
 * lightuserdata,
 * which it treats as a lua_CFunction.  It then pops this
 * lightuserdata and calls
 * @code{lua_pushcclosure(L, &luaS_call_gate, count);}, where
...
int luaS_pushcclosure(lua_State *L, lua_CFunction func,
...
static int luaS_pushSafeCFunction_(lua_State *L) {
   ...
   size_t const size = offset + (func->num_types >= 0 ?
                                 func->num_types : 0);
...
bool luaS_pushSafeCFunction(lua_State *L,
                            lua_CFunction func,
                            void *ud,
                            ssize_t num_types,
                            int max_stack_slots,
...
lua_State *luaS_newstate(void) {
   ...
   int errorcode = lua_cpcall(L, &luaS__registerclosure_, &luaS__registerclosure);
#else
